(() => {
    ...
    const page = ds.pages.getFocusedPage()
    ...
    const sections = _(ds.components.getChildren(ds.pages.getFocusedPage()))
        .filter(c => componentEditorAPI.hasSectionBehaviors(c))
        .filter(c => !componentsDerivativeStateAPI.isHiddenInCurrentBreakpoint(c))
        .filter(c => !isVerticalSection(c))
        .sortBy(v => ds.responsiveLayout.get(v).itemLayouts[0].gridArea.rowStart)
        .value()    
    const verticalSections = _(ds.components.getChildren(ds.pages.getFocusedPage()))
        .filter(c => componentEditorAPI.hasSectionBehaviors(c))
        .filter(c => !componentsDerivativeStateAPI.isHiddenInCurrentBreakpoint(c))
        .filter(c => isVerticalSection(c))
        .sortBy(v => ds.responsiveLayout.get(v).itemLayouts[0].gridArea.columnStart)
        .value()    
    // Page
